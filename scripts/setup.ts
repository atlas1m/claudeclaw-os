...
async function main() {
  ...
  const wantAgents = await confirm(
    'Set up specialist agents? (default: no — fork ships only the blank _template scaffold; create per-project agents later via npm run agent:create)',
    false,
  );
  if (wantAgents) {
    info('This fork removed the comms / content / ops / research scaffolds. Use npm run agent:create after setup completes to create per-project agents from the blank _template.');
  }
